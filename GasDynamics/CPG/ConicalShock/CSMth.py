...
import math
def residual_CSMth(B,th,M,k):
    CSprop=CSMB(M,B,k)
    res=CSprop[-2]-th
    return res
def CSMth(M,th,k=1.4):
    '''
    Gasdynamics Module
    ...
    if th==0:
        ...
        Best1=interpfn(1.01*th)
        Best2=interpfn(0.99*th)
        B=secant(residual_CSMth,[Best1,Best2],th,M,k)
        out=CSMB(M,B[0],k)
        return out
    else:
        print('error:value of th is impermissible')
